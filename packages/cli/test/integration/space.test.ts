...
describe('the node itself', () => {
  it('reports an un-enveloped /health with the frozen route count', async () => {
    ...
    expect(health.operations).toBe(196) /* 195→196 U2: skills.catalog — one more ROUTE */ /* 171→195 177: the 24 container HTTP routes — ROUTES, not catalog rows */ /* 168→171 148: spaces.workflows.* — ROUTES, not catalog rows */ /* 162→165 2026-08-16 W4/132: spaces.taskWorkflows.* */ /* 157→158 2026-08-13: #188 chat route */; // MEASURED off /health; +1 2026-08-13 tracking.pr.merge.
