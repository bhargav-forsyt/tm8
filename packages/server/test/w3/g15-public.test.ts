...
describe.sequential('W3.G15 public reserved and residual honesty', () => {
  ...
  it('reports the frozen HTTP catalog and implementation counts from production health', async () => {
    ...
    // all mounted.
    expect(OPERATIONS).toHaveLength(198); // +25 (177) containers, +1 (U2) skills.catalog
    ...
    expect(health).toMatchObject({
      ...
      operations: 196, // +24 (177): the container HTTP rows; +1 (U2): skills.catalog
      implemented: 194, // +24 (177) +1 (U2): all registered, all mounted
    });
  });
  ...
  it('has an EMPTY residual set: no v1 binding refuses an empty probe', () => {
    ...
    // 188 -> 189 (U2): skills.catalog answers for real, so it is not residual.
